...
@section('title', 'Laporan CCTV')
@section('page-title', 'Laporan CCTV')
@section('page-subtitle', 'Cetak & Filter Data CCTV RSUD')

@section('content')
<div class="bg-dark-bg-card rounded-lg shadow-md border-2 border-[#262626] p-6">
	<form action="{{ route('cctv.laporan') }}" method="GET" 
		...
		<div class="flex items-end space-x-2">
			...
			<a href="{{ route('cctv.laporan.print', request()->all()) }}" target="_blank"
		...
		<table class="min-w-full text-sm">
			...
			<tbody>
				...
				<tr class="border-t border-[#262626] hover:bg-[#262626] transition-colors">
					...
					<td class="py-3 px-4 font-medium text-[#A1A1A1]">{{ $cctv->ruangan }}</td>
					<td class="py-3 px-4 text-[#A1A1A1]">{{ $cctv->brand }}</td>
					<td class="py-3 px-4 text-[#A1A1A1]">{{ $cctv->kegiatan }}</td>
					<td class="py-3 px-4 text-[#A1A1A1]">{{ $cctv->tahun }}</td>
				</tr>
				@empty
				<tr>
