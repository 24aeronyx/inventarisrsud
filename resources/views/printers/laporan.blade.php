...
@section('title', 'Laporan Printer')
@section('page-title', 'Laporan Printer')
@section('page-subtitle', 'Cetak & Filter Data Printer RSUD')

@section('content')
<div class="bg-dark-bg-card rounded-lg shadow-md border-2 border-[#262626] p-6">
	<form action="{{ route('printer.laporan') }}" method="GET" 
		...
		{{-- Filter Jenis --}}
		<div>
			<label class="block text-dark-text-secondary mb-1">Jenis</label>
			<select name="jenis" class="w-full bg-[#262626] text-white rounded-lg py-2 px-3">
				<option value="">Semua</option>
                @foreach($jenis as $j)
		...
		<div class="flex items-end space-x-2">
			...
			<a href="{{ route('printer.laporan.print', request()->all()) }}" target="_blank"
		...
		<table class="min-w-full text-sm">
			...
			<tbody>
				...
				<tr class="border-t border-[#262626] hover:bg-[#262626] transition-colors">
					...
					<td class="py-3 px-4 text-[#A1A1A1]">{{ ucfirst($printer->jenis) }}</td>
					<td class="py-3 px-4 text-[#A1A1A1]">{{ $printer->kegiatan }}</td>
					<td class="py-3 px-4 text-[#A1A1A1]">{{ $printer->tahun }}</td>
				</tr>
				@empty
				<tr>
					<td colspan="5" class="py-8 px-4 text-center text-dark-text-secondary">
