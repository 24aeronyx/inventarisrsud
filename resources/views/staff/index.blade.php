...
<div class="bg-dark-bg-card rounded-lg shadow-md border-2 border-[#262626] p-6">
    <div class="flex items-center mb-6">
        <div class="relative w-full flex justify-between items-center flex-col md:flex-row ">
            <form action="{{ route('staff.index') }}" method="GET" class="lg:w-96 bg-[#262626] rounded-lg py-3 pl-3 pr-4 focus:outline-none focus:ring-2 focus:ring-white transition-colors duration-200 flex items-center">
                <iconify-icon icon="carbon:search" class="mr-3"></iconify-icon>
                <input type="text" name="query" value="{{ request('query') }}" placeholder="Cari staff..." class="outline-none bg-transparent text-white placeholder:text-dark-text-secondary">
            </form>
            <a href="{{ route('staff.create') }}" class="flex items-center bg-[#FFFFFF] text-[#0A0A0A] hover:bg-[#A1A1A1] focus:ring-[#FFFFFF] cursor-pointer rounded-lg py-3 px-4 transition-colors duration-200">
        ...
        <table class="min-w-full">
            ...
            <tbody>
                ...
                <tr class="border-t border-[#262626] hover:bg-[#262626] transition-colors duration-200">
                    <td class="py-3 px-4 font-medium text-[#A1A1A1]">{{ $loop->iteration + $staffs->firstItem() - 1  }}</td>
                    ...
                    <td class="py-3 px-4 space-x-8">
                        ...
                        <form action="{{route('staff.destroy', $staff->id)}}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="cursor-pointer" onclick="return confirm('Yakin ingin menghapus data staff ini?')">
                ...
                @empty
                <tr>
                    <td colspan="4" class="py-8 px-4 text-center text-dark-text-secondary">Tidak ada staff ditemukan.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
