	...
	<div class="bg-dark-bg-card rounded-lg shadow-md border-2 border-[#262626] p-6">
		<div class="flex items-center mb-6">
			<div class="relative w-full flex justify-between items-center flex-col md:flex-row ">
				<form action="{{ route('switch.index') }}" method="GET"
					...
					<input type="text" name="query" value="{{ request('query') }}" placeholder="Cari switch..."
				...
				<tbody>
					@forelse($switchhubs as $item)
						<tr class="border-t border-[#262626] hover:bg-[#262626] transition-colors duration-200">
							<td class="py-3 px-4 font-medium text-[#A1A1A1]">
								{{ $loop->iteration + $switchhubs->firstItem() - 1 }}
							</td>
							...
							<td class="py-3 px-4 space-x-6">
								<a href="{{ route('switch.show', $item->id) }}" title="Detail">
									<iconify-icon width="20" height="20" icon="lucide:info"
										class="text-[#A1A1A1] hover:text-blue-500 transition-colors duration-200"></iconify-icon>
								</a>
								...
								<form action="{{ route('switch.destroy', $item->id) }}" method="POST" class="inline">
									...
									<button type="submit" class="cursor-pointer" title="Hapus"
										onclick="return confirm('Yakin ingin menghapus data CCTV ini?')">
					...
					@empty
						<tr>
							<td colspan="5" class="py-8 px-4 text-center text-dark-text-secondary">Tidak ada CCTV ditemukan.
							</td>
						</tr>
					@endforelse
				</tbody>
