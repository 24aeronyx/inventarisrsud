...
@section('title', 'Laporan Switch')
@section('page-title', 'Laporan Switch')
@section('page-subtitle', 'Cetak & Filter Data Switch RSUD')

@section('content')
<div class="bg-dark-bg-card rounded-lg shadow-md border-2 border-[#262626] p-6">
	<form action="{{ route('switch.laporan') }}" method="GET" 
		...
		<div class="flex items-end space-x-2">
			...
			<a href="{{ route('switch.laporan.print', request()->all()) }}" target="_blank"
		...
		<table class="min-w-full text-sm">
			...
			<tbody>
				...
				<tr class="border-t border-[#262626] hover:bg-[#262626] transition-colors">
					...
					<td class="py-3 px-4 font-medium text-[#A1A1A1]">{{ $switchhub->ruangan }}</td>
					<td class="py-3 px-4 text-[#A1A1A1]">{{ $switchhub->brand }}</td>
					<td class="py-3 px-4 text-[#A1A1A1]">{{ $switchhub->kegiatan }}</td>
					<td class="py-3 px-4 text-[#A1A1A1]">{{ $switchhub->tahun }}</td>
				</tr>
				@empty
				<tr>
					<td colspan="4" class="py-8 px-4 text-center text-dark-text-secondary">
