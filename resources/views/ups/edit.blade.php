...
<div class="p-8 rounded-lg shadow-lg border-2 border-[#262626]">
    <h2 class="text-2xl font-bold text-[#FFFFFF] mb-6">Edit UPS</h2>
    <form action="{{ route('ups.update', $ups->id) }}" method="POST" autocomplete="off">
        @csrf
        @method('PUT')
                ...
                <select name="ruangan" id="ruangan" class="w-full bg-[#262626] text-white rounded-lg py-3 px-4 focus:outline-none focus:ring-2 focus:ring-white" required>
                    ...
                    @foreach($ruangan as $kategori => $list)
                        <optgroup label="{{ $kategori }}">
                            @foreach($list as $r)
                                <option value="{{ $r }}" {{ (old('ruangan', $ups->ruangan) == $r) ? 'selected' : '' }}>{{ $r }}</option>
                            @endforeach
                        </optgroup>
                    @endforeach
                </select>
        ...
        <div class="flex flex-col lg:flex-row gap-4 items-center">
            <div class="mb-4 w-full">
                    ...
                    <x-input 
                        ...
                        :value="old('kegiatan', $ups->kegiatan)" 
                        required 
                    />
...
@section('page-subtitle', 'Edit UPS RSUD')
