...
@section('title', 'Laporan UPS')
@section('page-title', 'Laporan UPS')
@section('page-subtitle', 'Cetak & Filter Data UPS RSUD')

@section('content')
<div class="bg-dark-bg-card rounded-lg shadow-md border-2 border-[#262626] p-6">
	<form action="{{ route('ups.laporan') }}" method="GET" 
		...
		<div class="flex items-end space-x-2">
			...
			<a href="{{ route('ups.laporan.print', request()->all()) }}" target="_blank"
		...
		<table class="min-w-full text-sm">
			...
			<tbody>
				...
				<tr class="border-t border-[#262626] hover:bg-[#262626] transition-colors">
					...
					<td class="py-3 px-4 font-medium text-[#A1A1A1]">{{ $ups->ruangan }}</td>
					<td class="py-3 px-4 text-[#A1A1A1]">{{ $ups->brand }}</td>
					<td class="py-3 px-4 text-[#A1A1A1]">{{ $ups->kegiatan }}</td>
					<td class="py-3 px-4 text-[#A1A1A1]">{{ $ups->tahun }}</td>
				</tr>
				@empty
				<tr>
